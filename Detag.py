...
filename = "Detag_" + date_today + ".xlsx"
...
for found in regions_names:
    ...
    if found in tagged_list and found not in protected_list and regions_delegate_authority[counter_regions] is True:
        ...
        ws.write(counter, 0, tags + region, shrink)
        ws.write(counter, 1, major_list[counter_regions])
        ws.write(counter, 2, minor_list[counter_regions])
        ws.write(counter, 3, regions_nation_counts[counter_regions])
        ws.write(counter, 4, regions_nations_cumulative[counter_regions])
        ws.write(counter, 5, regions_delegate_votes[counter_regions])
        ws.write(counter, 6, regions_links[counter_regions], shrink)
    ...
    # rows. This doesn't have the colour legend Kronos has because it's unneeded. See {'constant_memory': True})
    if counter < 14:
        if counter == 1:
        ...
        if counter == 10:
            ws.write(counter, 8, '[+] :', right)
            ws.write(counter, 9, 'Founder')
        if counter == 11:
            ws.write(counter, 8, '[~] :', right)
            ws.write(counter, 9, 'Executive Delegacy')
        if counter == 12:
            ws.write(counter, 8, '[!] :', right)
            ws.write(counter, 9, 'Founderless')
        if counter == 13:
            ws.write(counter, 8, '[!!] :', right)
            ws.write(counter, 9, 'Tagged "Invader"')
